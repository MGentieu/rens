...
if response.status_code == 200:
    ...
    filtered = [
        ...
        if start <= item.get("date", 0) <= end
    ]

    print(f"✅ {len(filtered)} éléments filtrés enregistrés dans 'resultats_filtres.json'")

    # Sauvegarde uniquement les résultats filtrés
    with open("resultats_filtres.json", "w", encoding="utf-8") as f:
